...
public class MaitreDuJeu {
    ...
    private int nbTours = 0; //nombre de tours Ã©coulÃ© dans la partie
    ...
    public MaitreDuJeu(ArrayList<Joueur> joueurs) {
        ...
        //crÃ©er autant de pile de parcelle qu'il y a de joueurs
    ...
    public static void main(String[] args) {
        ...
        while (mj.getCli() == null && mj.getServ() == null && !mj.getLocal()) {
            ...
            if (mj.getServ() != null) {
                ...
                mj.fenetre.getLauncher().setInfo("Tout les joueurs sont connectés débbut de la partie");
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                ...
                for (int i = 0; i < 3; i++) {
                    while (mj.getServ().getPseudo(i) == "unknow") {
                        mj.fenetre.getLauncher().setInfo("Tout les joueurs sont connectés début de la partie");
                    }
    ...
    //gére la premiére  phase du jeu les enchÃ¨res pour les parcelles
    ...
    public void enchereParcelleServeur() {
        ...
        for (int i = 0; i < joueurs.size(); i++) {
            ...
            if (j_actif.getPseudo().equals(pseudoJoueur)) {
                ...
                System.out.println("Parcelle reÃ§ue");
                j_actif.setParcelleMain(pChoisie);
                serv.sendParcelleChoisi(j_actif, pChoisie);
    ...
    public void enchereParcelleClient() {
        ...
        for (int i = 0; i < joueurs.size(); i++) {
            ...
            //si quelqu'un d'autre Ã  jouer récupéré¨re son choix
            else {
                System.out.println(" A lui !" + j_actif.getPseudo());
    ...
    //gerere la  cinquieme phase du jeu
    private void irriguationComplementaire() {
        ...
        for (int i = 0; i < joueurs.size(); i++) {
            j_actif = joueurs.get(i);
            refreshInfo("Irriguation");
            if (j_actif.isCanalComplementaire()) { //si la personne actif a un canal complÃ©mentaire
                if (!adeposer) { //si personne n'a dÃ©ja posÃ© son canal durant ce tour de jeu
    ...
    private void initialisationPileParcelles() {
        ...
        //Si on est Ã  4 joueurs on retire une parcelle du jeu
        if (nbPile == 4) {
            parcelles.remove(0);
        }
    ...
    private void jouerPartieLocal(ArrayList<Joueur> listeJoueurs) {
        ...
        do {
            ...
            //Phase Irrigation complÃ©mentaire
